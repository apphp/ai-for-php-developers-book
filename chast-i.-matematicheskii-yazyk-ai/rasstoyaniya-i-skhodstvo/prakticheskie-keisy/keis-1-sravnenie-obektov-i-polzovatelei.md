...
// Результат:
// $userANorm ≈ [0.14, 0.22, 0.21]
// $userBNorm ≈ [0.48, 0.44, 0.37]
```
...
// Результат: 0.39
// Объяснение: d = (0.14 − 0.48)^2 + (0.22 − 0.44)^2 + (0.21 − 0.37)^2 ​ ≈0.39
```

Малое значение означает, что пользователи похожи по совокупности признаков. В нашем случае расстояние ≈ 0.39 означает, что пользователи умеренно различаются по возрасту, доходу и активности покупок. Именно по этому принципу работают:
...
// Результат: 0.373
// Объяснение: 
// score = (0.14 * 0.2) + (0.22 * 0.9) + (0.21 * 0.7) = 0.028 + 0.198 + 0.147 = 0.373
```

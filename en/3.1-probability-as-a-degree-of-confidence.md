...
Many models ([logistic regression](https://chatgpt.com/ai-for-php-developers-intuitive-and-practical/introduction/glossary.md#logistic-regression), neural networks) use the [softmax](https://chatgpt.com/ai-for-php-developers-intuitive-and-practical/introduction/glossary.md#softmax) function at the output layer:
...
In many machine learning models, the output is not a probability but a set of _scores_ ([logits](https://chatgpt.com/ai-for-php-developers-intuitive-and-practical/introduction/glossary.md#logits)). These are simply numbers representing the model's relative confidence in each option. They can be positive, negative, large, or small, and by themselves they are not probabilities.
...
function softmax(array $scores): array {
    ...
    foreach ($scores as $key => $value) {
        ...
        $expValues[$key] = $exp;
        $sum += $exp; // накапливаем сумму для нормализации
    }

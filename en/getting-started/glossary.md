...
Data consists of observations represented in numerical form. In machine learning, data is almost always interpreted as points in a multidimensional space, where each coordinate corresponds to a feature.

Texts, images, events, and users all ultimately become vectors of numbers.
...
Embeddings are dense vector representations of objects – such as words, sentences, documents, or images – that encode their semantic meaning.

Unlike the Bag-of-Words approach, embeddings:

* have a fixed dimensionality
* capture semantic relationships
* allow comparison based on meaning rather than simple word overlap

Modern embeddings are typically generated by neural networks and are widely used in search engines, recommendation systems, and Retrieval-Augmented Generation (RAG) systems.

### Embedding Vector

An embedding vector is a numerical vector representation of an object in a feature space.

Embedding models transform words, text, images, or users into vectors such that objects with similar meanings are located close to one another in that space.

Embedding vectors are widely used in:
...
The similarity between embedding vectors is typically measured using cosine similarity or the distance between vectors.

Intuitively, an embedding vector is the set of coordinates that represents an object's meaning in a high-dimensional space.

### Embedding Models

Embedding models are machine learning models that convert objects – such as words, text, images, or users – into numerical vectors.

Objects with similar meanings are mapped to nearby vectors in the feature space.
...
The similarity between embeddings is typically measured using cosine similarity or the distance between vectors.

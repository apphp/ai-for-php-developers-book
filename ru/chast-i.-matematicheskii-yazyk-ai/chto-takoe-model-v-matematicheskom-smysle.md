...
Ошибка – это функция (общепринятое название - [Loss function](../vvedenie/zaklyuchitelnye-materialy/glossarii.md#loss-funkciya)), которая сравнивает предсказание модели с реальным значением и возвращает число, показывающее, насколько сильно модель ошиблась. Чем меньше это число, тем лучше модель.

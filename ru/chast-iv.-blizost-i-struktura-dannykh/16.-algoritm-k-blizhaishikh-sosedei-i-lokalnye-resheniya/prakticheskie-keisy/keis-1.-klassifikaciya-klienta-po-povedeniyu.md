---
description: Понимаем k-NN на простом поведенческом примере.
---
...
* каждый пользователь – это точка вида \[visits, averageTime]
...
В нашем случае большинство (три из трёх) ближайших пользователей относятся к классу _engaged_, поэтому именно его мы и получаем в качестве предсказания.
...
Поняв этот пример, вам будет легче понять, как работают более сложные алгоритмы, которые тоже опираются на понятия близости, соседства и локальной структуры данных – просто делают это менее явно.

...
В этой главе мы разберём, как дерево решений последовательно делит пространство признаков, почему в основе этого процесса _часто_ лежит энтропия, что такое information gain и почему именно эти идеи делают деревья такими объяснимыми.
...
Представим двухмерный случай: у нас есть признаки $$x1$$ и $$x2$$. Дерево решений будет проводить вертикальные или горизонтальные линии (параллельные осям признаков), разделяя плоскость на прямоугольные области. В каждой такой области решение становится более однородным.
...
Кроме того, дерево позволяет оценивать важность признаков. Чем чаще и выше по дереву используется признак, тем больший вклад он вносит в итоговое решение. При этом такие оценки могут быть смещены, например в пользу признаков с большим числом уникальных значений или коррелированных признаков. Эта идея потом развивается в ансамблях, таких как Random Forest и Gradient Boosting.
...
* когда важна максимальная точность любой ценой

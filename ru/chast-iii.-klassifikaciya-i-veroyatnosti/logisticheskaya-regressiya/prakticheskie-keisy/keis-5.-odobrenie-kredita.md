...
В задачах кредитования модель редко отвечает на вопрос "да или нет" напрямую.

Гораздо важнее другое: насколько рискован клиент.

Именно поэтому логистическая регрессия здесь используется очень часто – она дает не просто решение, а вероятность дефолта, с которой уже можно работать.
...
* 1 – кредит можно одобрить (надежный клиент)
* 0 – кредит лучше не выдавать (высокий риск)
...
Модель должна определить, насколько он надежен.
...
Здесь $$p$$  – это вероятность того, что клиент относится к "хорошему" классу (или наоборот, к дефолту – в зависимости от формулировки задачи).

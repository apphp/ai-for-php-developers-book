...
Он прост в установке и запуске, но при этом очень наглядно показывает, как работает модель – и где она начинает ломаться.
...
* затем нужно разделить датасет на две подвыборки (subsets): train и test, но в нашем случае это уже сделано за нас самим MNIST
...
Для данной задачи с MNIST 0 vs 1 (очень простой) – классы хорошо разделимы, поэтому даже линейная модель даёт \~99% точности.
...
Не смотря на то, что для данной задачи 0 vs 1  – классы хорошо разделимы, тем не менее RubixML может дать результат чуть лучше (99.95%) за счёт более стабильной реализации и оптимизаций.
...
Но у неё есть фундаментальное ограничение (см. выше).

...
Начнем с привычного. Если мы много раз подбрасываем честную монетку (такую, у которой в долгой серии бросков ни одна сторона не выпадает чаще другой), то ожидаем, что орёл будет выпадать примерно в половине случаев. Формально это записывается так:

$$
P(\text{орел}) = \frac{1}{2}
$$

Здесь вероятность интерпретируется как предел относительной частоты при большом числе экспериментов. Это так называемая _частотная (frequentist) интерпретация_.
...
То же самое в машинном обучении:
...
Важно: вероятность здесь, в байесовской интерпретации, не свойство объекта, а свойство нашей модели и нашего знания. И при другой модели или других данных эта оценка могла бы быть иной.
...
В байесовской интерпретации это число читается как степень уверенности, где:
...
В байесовской интерпретации значения 0 и 1 означают полную (догматическую) уверенность и на практике почти не используются.

В машинном же обучении модель обычно выдает промежуточные значения – степень уверенности, а не жесткий ответ "да" или "нет".
...
И вот здесь впервые появляется формула Байеса.

{% hint style="info" %}
Формула названа в честь английского математика и священника [Томаса Байеса](https://ru.wikipedia.org/wiki/%D0%91%D0%B0%D0%B9%D0%B5%D1%81,_%D0%A2%D0%BE%D0%BC%D0%B0%D1%81) (XVIII век). Он впервые сформулировал идею обновления вероятности при появлении новых данных. Позже [Пьер-Симон Лаплас](https://ru.wikipedia.org/wiki/%D0%9B%D0%B0%D0%BF%D0%BB%D0%B0%D1%81,_%D0%9F%D1%8C%D0%B5%D1%80-%D0%A1%D0%B8%D0%BC%D0%BE%D0%BD) независимо развил и обобщил этот подход, превратив его в полноценный инструмент вероятностного вывода.
{% endhint %}
...
Для наглядности используем обозначения: "Б" – болен, "З" – здоров, а "+" – положительный тест.
...
Во многих моделях ([логистическая регрессия](../../vvedenie/zaklyuchitelnye-materialy/glossarii.md#logisticheskaya-regressiya), нейросети) на выходе используется функция [softmax](../../vvedenie/zaklyuchitelnye-materialy/glossarii.md#softmax):
...
Это нормализованные вероятности модели; они не обязательно являются хорошо калиброванными оценками реальных частот. В общем строго говоря, это вероятности модели, а не объективные вероятности мира.&#x20;

Смысл в том, чтобы привести оценки модели к форме степеней уверенности модели, с которыми удобно работать дальше – сравнивать, устанавливать пороговые значения и интерпретировать.

#### Пример вероятности с softmax

Во многих моделях машинного обучения выходом являются не вероятности, а так называемые _оценки_ ([logits](../../vvedenie/zaklyuchitelnye-materialy/glossarii.md#logits)). Это просто числа, отражающие относительную уверенность модели в каждом варианте. Они могут быть любыми – положительными, отрицательными, большими или маленькими – и сами по себе не интерпретируются как вероятность.
...
Не менее важно понимать: вероятность сама по себе ничего не решает. Решения принимаются на уровне  бизнес-логики.

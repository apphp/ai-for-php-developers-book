...
Во-первых, здесь нет обучения в оптимизационном смысле: нет итераций, функции потерь и настройки параметров. Модель полностью определяется частотами в обучающей выборке.
...
* кодирование и расширение категориальных признаков(масштабирование данных)

В примере ниже используется примитивный fallback (`?? 1`) для избежания $$log(0)$$. Это не является корректным Laplace-сглаживанием и сделано исключительно для наглядности и компактности примера.

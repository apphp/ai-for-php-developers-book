...
# 3.3 Почему наивный Байес работает
...
* $$P(X \mid C)$$ – правдоподобие (насколько наблюдения характерны для класса)
* $$P(X)$$ – нормализующий коэффициент

В классификации нас обычно не интересует точное значение $$P(X)$$, потому что оно не зависит от класса и поэтому сокращается при сравнении. Мы сравниваем относительные значения:

$$
P(C \mid X) \propto P(X \mid C) \ P(C)
$$

### Как из Байеса получается "наивный" Байес

Вот здесь происходит главный "трюк" модели.

#### 1. Раскрываем, что такое X

В реальности объект – это набор признаков (реальные данные состоят не из одного признака, а из множества):
...
Тогда формула Байеса становится:

$$
P(C \mid x_1, ..., x_n) = \frac{ P(x_1, ..., x_n \mid C) \cdot P(C) }{ P(x_1, ..., x_n) }
$$
...
его почти невозможно посчитать напрямую – нужно учитывать все зависимости между признаками и это очень сложно сделать, так как нужно слишком много данных.

#### 3. Наивное предположение

Отсюда мы делаем упрощение:

**признаки независимы при условии класса**

$$
P(x_1, \ldots, x_n \mid C) = \prod_{i=1}^{n} P(x_i \mid C)
$$

В этом и заключается вся "наивность".
...
* $$C$$ – класс (например, 0 или 1)
* $$X=(x_1​,...,x_n​)$$ – все признаки (например, слова, длина текста, пиксели и т.д.)
* $$P(C)$$ – априорная вероятность класса
* $$\prod_{i=1}^{n} P(x_i \mid C)$$ – "насколько признаки похожи на класс"
* $$P(X)$$ – нормализующий множитель (часто можно игнорировать при сравнении классов)

#### 5. Упрощение для классификации

Поскольку $$P(X)$$ одинаково для всех классов, его можно убрать и таким образом мы получим:

$$
P(C \mid X) \propto P(C) \cdot \prod_{i=1}^{n} P(x_i \mid C)
$$

Вот это и есть правило классификации наивного Байеса.
...
Классический Байес превращается в наивный в момент, когда:

> совместная вероятность $$P(x_1​,...,x_n​∣C)$$ заменяется на произведение независимых вероятностей признаков
...
Мы не спрашиваем: "Какова точная вероятность, что это спам?"

Мы спрашиваем: "Спам или не спам?"

#### **2. Ошибки часто компенсируют друг друга**

Ошибки из-за зависимости признаков часто распределяются по всем классам примерно одинаково (если структура зависимостей похожа внутри каждого класса). В итоге максимальное значение остается у правильного класса.
...
* не требует итеративной оптимизации,
...
<div align="left"><figure><img src="../../.gitbook/assets/3.3-1_geometry-of-naive-bayes.png" alt=""><figcaption><p>Рис. 3.3-1. Геометрия наивного Байеса</p></figcaption></figure></div>
...
<div align="left"><figure><img src="../../.gitbook/assets/3.3-2_voting-of-features.png" alt=""><figcaption><p>Рис. 3.3-2. Голосование признаков</p></figcaption></figure></div>
...
С точки зрения вычислений, наивный Байес сводится к сумме лог-вероятностей.

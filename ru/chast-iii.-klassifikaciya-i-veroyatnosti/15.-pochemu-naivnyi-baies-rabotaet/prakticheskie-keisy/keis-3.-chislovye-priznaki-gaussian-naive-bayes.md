...
В этом кейсе мы рассмотрим, как наивный Байес работает с такими признаками. Для этого используется вариант модели, называемый [Gaussian Naive Bayes](../../../vvedenie/glossarii.md#gaussian-naive-bayes), где предполагается, что значения признаков внутри каждого класса распределены нормально.

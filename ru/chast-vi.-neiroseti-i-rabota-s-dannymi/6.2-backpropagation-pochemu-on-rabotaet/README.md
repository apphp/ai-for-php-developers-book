...
Для сигмоиды есть готовая формула:
...
Подставляем значение выхода нейрона:
...
\[\
\boxed{\frac{\partial L}{\partial w}\approx-0.212}\
]
...
Градиент оказался отрицательным:

\[\
\frac{\partial L}{\partial w}\approx-0.212\
]

Это говорит о том, что **увеличение веса уменьшит ошибку**.

Например, при скорости обучения (\eta=0.1):
...
Если вы разобрали этот числовой пример, то увидели, что для одного нейрона вычисление градиента не представляет особой сложности.&#x20;

Может показаться, что для глубокой нейронной сети задача станет практически невыполнимой. Однако это не так. Независимо от того, содержит сеть один нейрон или миллионы параметров, backpropagation использует один и тот же механизм – цепное правило. Единственное отличие заключается в том, что цепочка зависимостей становится значительно длиннее, а вычислительный граф – гораздо больше.

...
Но прежде чем двигаться дальше, полезно реализовать классический перцептрон самостоятельно. Благодаря этому станет понятно, насколько проста его внутренняя логика и почему одной такой модели оказывается недостаточно.
...
Несмотря на успешное обучение, ничего принципиально нового не произошло: модель лишь подобрала более удачное положение своей разделяющей прямой. Если задача требует нелинейной границы, никакое количество эпох обучения уже не поможет.

Чтобы научиться решать более сложные нелинейные задачи, необходимо объединить несколько нейронов в слои. Но прежде нужно понять, почему простого добавления новых линейных слоев недостаточно.
...
На первый взгляд это может показаться неожиданным. Однако композиция линейных преобразований сама остается линейным преобразованием. Поэтому простое добавление новых линейных слоев не увеличивает выразительность модели.
...
Теперь остается разобраться, какие функции активации используются на практике и почему именно они стали стандартом. Таким образом, у нас есть все необходимые строительные блоки: линейное преобразование и нелинейная функция активации. Осталось объединить их в несколько слоев.&#x20;
...
Возникает закономерный вопрос: насколько мощной может быть такая архитектура? Ответ дает одна из самых известных теорем теории нейронных сетей.

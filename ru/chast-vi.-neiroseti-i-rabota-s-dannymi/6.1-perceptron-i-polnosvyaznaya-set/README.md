...
Несмотря на то, что современные архитектуры выглядят невероятно сложными, в их основе  лежит один и тот же принцип: вычисление взвешенной суммы входов с последующим применением функции активации. Классический перцептрон использовал [пороговую функцию](../../vvedenie/glossarii.md#threshold-function-porogovaya-funkciya), тогда как современные нейронные сети используют более сложные функции, о которых мы поговорим чуть позже в этой главе.
...
Каждый фактор имеет свою "важность". Доход может быть важнее стажа, а просрочки – сильным отрицательным фактором. Каждый фактор сначала умножается на свой вес, после чего результаты складываются. Если итоговый "балл" превышает некоторый порог – кредит одобряется.
...
Перцептрон был предложен [Фрэнком Розенблаттом](https://ru.wikipedia.org/wiki/%D0%A0%D0%BE%D0%B7%D0%B5%D0%BD%D0%B1%D0%BB%D0%B0%D1%82%D1%82,_%D0%A4%D1%80%D1%8D%D0%BD%D0%BA). Это была одна из первых моделей, чьё создание было вдохновенно тем как устроены и работают нейроны мозга.

{% hint style="info" %}
В 1958 году Розенблатт опубликовал работу [_The Perceptron: A Probabilistic Model for Information Storage and Organization in the Brain_](https://homepages.math.uic.edu/~lreyzin/papers/rosenblatt58.pdf), благодаря которой эта модель получила широкую известность.
{% endhint %}
...
Пока мы описали только то, как перцептрон принимает решение. Следующий шаг – понять, как эта идея записывается математически и какие параметры определяют поведение модели.
...
Именно эта формула, при всей своей простоте, является одной из самых важных во всей книге. В разных моделях будут меняться функции активации, способы обучения и архитектура сети, но вычисление линейной комбинации останется практически неизменным.
...
В двумерном случае это прямая. В трехмерном – плоскость. В n-мерном – гиперплоскость.
...
Возникает естественный вопрос: а что произойдет, если одной прямой уже недостаточно?&#x20;

Именно здесь проявляется главное ограничение перцептрона: он способен разделять данные только в тех случаях, когда их можно корректно разделить одной прямой. Что произойдет, когда данные устроены сложнее, мы рассмотрим далее.
...
Теперь становится понятно, почему перцептрон называют линейным классификатором. Он не умеет строить криволинейные границы – только одну гиперплоскость.
...
словами, то это означает – мы делим пространство линейной границей.
...
Пока мы рассматривали перцептрон как готовую математическую модель с уже заданными весами. Но на практике веса не задаются вручную – модель подбирает их уже в процессе обучения на основе обучающих примеров.

#### Перцептрон как алгоритм обучения

Также важно понимать: перцептрон – это не только математическая формула, но и алгоритм обучения, который определяет, как изменяются его веса.

При ошибке модель корректирует веса в направлении, которое должно уменьшить эту ошибку:
...
Не стоит пытаться сразу запомнить эту формулу. Пока важно понять только общий принцип: если модель ошиблась, веса немного изменяются так, чтобы в следующий раз вероятность правильного ответа стала выше.
...
С каждым проходом по данным граница постепенно "подстраивается". Однако само обучение не меняет природу модели. Как бы долго ни обучался перцептрон, он по-прежнему остается линейным классификатором. Именно поэтому полезно ненадолго отвлечься от самого перцептрона и посмотреть, какую роль линейная комбинация играет в других моделях машинного обучения.

### Линейная комбинация – общая идея

Как мы уже видели выше, перцептрон начинает работу с вычисления:
...
Именно эта линейная комбинация лежит в основе не только перцептрона, но и линейной регрессии, логистической регрессии и большинства современных нейронных сетей. Различия между моделями появляются уже после этого шага – в функции активации и способе обучения.

### Классический перцептрон как линейный классификатор
...
Геометрически – разделяем пространство гиперплоскостью. Именно в этом и проявляется главное достоинство и одновременно главное ограничение перцептрона.

#### Ограничение

Как вы уже знаете, перцептрон умеет решать только [линейно разделимые задачи](https://ru.wikipedia.org/wiki/%D0%9B%D0%B8%D0%BD%D0%B5%D0%B9%D0%BD%D0%B0%D1%8F_%D1%81%D0%B5%D0%BF%D0%B0%D1%80%D0%B0%D0%B1%D0%B5%D0%BB%D1%8C%D0%BD%D0%BE%D1%81%D1%82%D1%8C).

Например, XOR (как было упомянуто выше) он решить не может.

<div align="left"><figure><img src="../../.gitbook/assets/6.1-4_xor-problem.png" alt=""><figcaption><p>Рис. 6.1-4. Проблема XOR</p></figcaption></figure></div>

XOR невозможно разделить одной гиперплоскостью, для его решения необходима нелинейность.&#x20;

Возникает естественный вопрос: можно ли объединить несколько перцептронов таким образом, чтобы вместе они смогли описывать более сложные границы? Интересно, что в конечном итоге именно поиск ответа на этот вопрос и привел к созданию многослойных нейронных сетей.

Однако, прежде чем двигаться дальше, давайте реализуем классический перцептрон самостоятельно. Благодаря этому станет понятно, насколько проста его внутренняя логика и почему одной такой модели оказывается недостаточно.

### Перцептрон на чистом PHP

Для начала, давайте реализуем перцептрон в минимальной форме:
...
class Perceptron {
    ...
    public function train(array $X, array $y, int $epochs = 100): void {
        // Правило обновления Розенблатта 
        // применяется к набору данных в несколько проходов
        for ($e = 0; $e < $epochs; $e++) {
...
Несмотря на успешное обучение, модель лишь изменила положение своей разделяющей гиперплоскости. Она не стала "умнее" в математическом смысле – её возможности по-прежнему ограничены линейной границей. Если задача потребует нелинейного разделения, никакое количество эпох обучения уже не поможет.

Чтобы научить модель решать более сложные нелинейные задачи, необходимо объединить несколько нейронов в слои. Но прежде нужно понять, почему простого добавления новых линейных слоев недостаточно.

### Почему нужна нелинейность

Давайте рассмотрим следующую идею: если одного перцептрона недостаточно, может быть, достаточно соединить несколько линейных слоев последовательно?
...
Как видите, мы снова получили линейную модель.

На первый взгляд это может показаться неожиданным. Однако композиция линейных преобразований сама остается линейным преобразованием. Поэтому простое добавление новых линейных слоев не может увеличить выразительность модели.

Следовательно:

> Несколько линейных (точнее – аффинных) слоев без нелинейности эквивалентны одному аффинному преобразованию.

Именно поэтому добавление активации – не "декорация", а математическая необходимость.&#x20;
...
Теперь остается разобраться, какие функции активации используются на практике.
...
#### 2. [ReLU](https://en.wikipedia.org/wiki/Rectified_linear_unit)
...
#### 3. [Hyperbolic Tangent](https://en.wikipedia.org/wiki/Hyperbolic_functions)
...
### Полносвязная сеть (MLP или многослойный перцептрон)

MLP ([Multi-Layer Perceptron](https://en.wikipedia.org/wiki/Multilayer_perceptron)) – это несколько слоев искусственных нейронов. Несмотря на историческое название, современные MLP используют не классические перцептроны со ступенчатой функцией, а [дифференцируемые](../../vvedenie/glossarii.md#differenciruemye-neirony) нейроны с нелинейными функциями активации (для нас в данном случае это не принципиально):
...
Каждый нейрон связан со всеми нейронами предыдущего слоя – отсюда и название "полносвязная".
...
Каждый скрытый слой можно рассматривать как набор "детекторов" различных признаков. Обнаруженные признаки передаются следующему слою, который объединяет их в более сложные структуры. Этот процесс повторяется слой за слоем, пока сеть не сформирует итоговое решение.

Тут может возникнуть закономерный вопрос: насколько мощной должна быть такая архитектура? Ответ на это дает одна из самых известных теорем теории нейронных сетей.
...
class SimpleMLP {
    ...
    public function forward(array $x): float {
        ...
        // По очереди вычисляем значение каждого скрытого нейрона
        ...
        // Сохраняем активации скрытого слоя,
        // чтобы их можно было посмотреть после прямого прохода
        $this->hidden = $hidden;

        $output = 0.0;

        // Выход сети — взвешенная сумма активаций скрытого слоя
...
Для простоты этот пример не использует смещения (bias). В реальных нейронных сетях каждый нейрон обычно имеет собственный bias.
...
Перцептрон и логистическая регрессия используют одну и ту же линейную комбинацию признаков. Главное отличие заключается в функции активации и способе обучения. Перцептрон использует ступенчатую функцию и [правило обучения Розенблатта](../../vvedenie/glossarii.md#pravilo-obucheniya-rozenblatta), тогда как логистическая регрессия использует сигмоиду и оптимизирует логарифмическую функцию потерь ([log loss](../../vvedenie/glossarii.md#log-loss-logarifmicheskaya-funkciya-poter)).
...
Именно добавление нелинейности превращает простую линейную алгебру в [универсальный аппроксиматор](../../vvedenie/glossarii.md#universalnyi-approksimator).

В следующей главе мы подробно разберем backpropagation – механизм, который позволяет  сетям обучаться эффективно.

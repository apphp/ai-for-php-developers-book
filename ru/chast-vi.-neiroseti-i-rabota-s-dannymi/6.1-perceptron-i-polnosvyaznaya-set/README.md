...
Именно эта формула является одной из самых важных во всей книге. В разных моделях будут меняться функции активации, способы обучения и архитектура сети, но вычисление линейной комбинации останется практически неизменным.
...
Возникает естественный вопрос: а что произойдет, если одной прямой уже недостаточно? Это очень важный вопрос, который определяет главное ограничение перцептрона. Мы его рассмотрим далее.
...
Пока мы рассматривали перцептрон как готовую математическую модель. Но остается главный вопрос: откуда вообще берутся веса? Ответ – они подбираются во время обучения.
...
Геометрически – разделяем пространство гиперплоскостью. Именно здесь проявляется главное достоинство и одновременно главное ограничение перцептрона.
...
Возникает естественный вопрос: можно ли объединить несколько перцептронов таким образом, чтобы вместе они смогли описывать более сложные границы? Интересно, что именно поиск ответа на этот вопрос и привел к созданию многослойных нейронных сетей.

Но прежде чем двигаться дальше, давайте реализуем классический перцептрон самостоятельно. Благодаря этому станет понятно, насколько проста его внутренняя логика и почему одной такой модели оказывается недостаточно.

---
description: Геометрия смысла и семантический поиск.
---
...
Эмбеддинги ([embeddings](../../vvedenie/glossarii.md#embeddings-embeddingi)) меняют сам способ мышления.

Вместо вопроса "какие признаки у этого объекта?" мы задаем другой: "где он расположен в пространстве смысла – или, точнее, в пространстве статистических отношений, которые модель научилась интерпретировать как смысловую близость?".

Эмбеддинг – это отображение объекта (слова, фразы, документа, пользователя, товара) в точку многомерного непрерывного пространства. Обычно это вектор из десятков, сотен или тысяч чисел:
...
<div align="left"><figure><img src="../../.gitbook/assets/5.3-1_embedding-space-overview.png" alt=""><figcaption><p>Рис. 5.3-1. Обзор эмбеддинг-пространства</p></figcaption></figure></div>

### Геометрия вместо логики

Ключевая идея эмбеддингов звучит почти философски: смысл с это не правило, а положение. Модель больше не проверяет набор условий – она измеряет взаимное расположение объектов.
...
Или эквивалентную ему косинусное расстояние (или, точнее, меру различия):

$$
\text{dist}(\vec{a}, \vec{b}) = 1 - \text{sim}(\vec{a}, \vec{b})
$$

Важно заметить: нас интересует угол между векторами, а не их длина. Во многих практических задачах длина вектора либо нормализуется, либо играет второстепенную роль по сравнению с направлением. Однако в некоторых [эмбеддинг-моделях](../../vvedenie/glossarii.md#embeddings-modeli) норма вектора тоже может содержать полезную информацию.
...
На первый взгляд эмбеддинги выглядят как магия: модель выдает набор чисел, а мы внезапно начинаем искать по смыслу, а не по словам.

Магии здесь нет. Есть статистика, оптимизация и контекст.

Эмбеддинг-модели обучаются так, чтобы объекты, встречающиеся в похожих контекстах, оказывались ближе друг к другу. Это обобщение старой идеи дистрибутивной семантики: \
"Смысл слова определяется его окружением". В классических моделях это достигается через статистику совместной встречаемости слов, а в [transformer-моделях](../../vvedenie/glossarii.md#transformer-modeli) – через предсказательные задачи и контекстное внимание (attention).

Если слово "лук" часто встречается рядом со словами "стрела", "тетива", "охота", то модель поймёт, что речь идёт об оружии.\
Если же рядом встречаются слова "салат", "овощи", "жарить", то модель научится отличать значение, связанное с едой.
...
Самое важное свойство эмбеддинг-пространства – непрерывность.

В классических подходах категории дискретны: либо слово совпало, либо нет. Либо класс A, либо класс B.

В эмбеддинг-пространстве возможны полутона.
...
Эмбеддинг-пространства обладают удивительными геометрическими эффектами.
...
Важно понимать: эмбеддинги не хранят знания в виде явных символических фактов или правил. Вместо этого они кодируют статистические отношения между объектами (хранят геометрию отношений).

### От эмбеддингов к семантическому поиску

Семантический поиск – это прямое следствие embedding-подхода.

Вместо того чтобы искать документы по совпадению слов, мы:

1. Преобразуем запрос пользователя в [embedding-вектор](../../vvedenie/glossarii.md#embedding-vektor).
...
### Почему семантический поиск лучше поиска по ключевым словам

Cемантический поиск ([semantic search](../../vvedenie/glossarii.md#semantic-search)) устойчив к:
...
Хотя качество такого поиска сильно зависит от модели, домена и качества эмбеддингов.
...
Здесь эмбеддинги становятся универсальным представлением, через которое разные типы данных можно сравнивать геометрически.

### Эмбеддинги как универсальный интерфейс

Один и тот же эмбеддинг-подход применяется для:
...
### Минимальный пример: эмбеддинг как вектор чисел (PHP)

Представим, что у нас уже есть функция, которая превращает текст в эмбеддинг. Не важно, как  именно она устроена внутри – сейчас нас интересует _результат_.
...
#### Семантический поиск в несколько строк
...
Эмбеддинги – это переход от дискретных признаков к непрерывной геометрии.
...
Семантический поиск – лишь первый и самый очевидный шаг. Дальше идут кластеризация знаний, рекомендательные системы, [RAG](../../vvedenie/glossarii.md#retrieval-arkhitektury) и гибридные модели мышления.

В следующих главах мы будем использовать эмбеддинги как базовый строительный блок, почти как фундаментальную абстракцию для представления смысла и близости – не задумываясь о том, как именно внутри устроена эта геометрия, но постоянно пользуясь ее свойствами.

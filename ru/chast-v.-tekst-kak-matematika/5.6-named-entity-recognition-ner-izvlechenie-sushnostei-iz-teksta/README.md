...
Модель же видит последовательность токенов. Задача [NER](../../vvedenie/glossarii.md#named-entity-recognition-ner-raspoznavanie-imenovannykh-sushnostei) – научить модель выделять сущности внутри текста.
...
Но вместе "Bank of America" образует единую сущность типа ORG.

Поэтому NER требует моделей, которые учитывают последовательность: RNN, LSTM, Transformers. Сегодня – чаще всего Transformers.

### BIO-разметка

Чтобы модель понимала границы сущностей, часто используется схема BIO (одна из наиболее распространённых схем разметки, хотя существуют и другие варианты):
...
Это уже не классификация всего текста целиком. Это классификация каждого токена в последовательности (sequence labeling).
...
Поэтому продвинутые модели (например, CRF) учитывают зависимости между соседними метками и оценивают согласованность всей последовательности предсказаний.
...
Для примера будем использовать любою модель, которая вам больше подходит.&#x20;

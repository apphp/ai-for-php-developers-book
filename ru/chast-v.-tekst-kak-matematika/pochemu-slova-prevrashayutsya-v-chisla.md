...
Ответ одновременно простой и немного обидный: машина ничего не понимает. Она умеет считать, т.е. оперировать числами и воспроизводить статистические закономерности. Под "пониманием" здесь имеется в виду человеческое, осмысленное понимание, а не формальная обработка чисел. Наша задача – придумать такое числовое представление текста, при котором подсчёты начинают коррелировать со смыслом.
...
Большинство алгоритмов машинного обучения (в том числе те, с которыми мы сталкивались раньше) принимают на вход вектор чисел:
...
Необходимо сразу принять одну неприятную мысль: не существует некого "правильного" отображения текста в числа. Есть лишь более или менее полезные представления, которые хорошо работают для конкретной задачи.

### От слов к координатам

Начнём с интуиции. Представим, что у нас есть очень простой мир, в котором всего три слова:\
"кот", "пёс" и "автомобиль".
...
Отсюда появляется ключевая идея: числовые значения должны отражать существенные свойства объектов, а не выступать лишь в роли формальных идентификаторов.

### Признаки вместо слов

Когда мы говорим "признак", мы имеем в виду измеряемую характеристику объекта. Для текста объект – это слово, предложение или документ, а признаки – это ответы на вопросы следующего вида (не обязательно интерпретируемые человеком напрямую):
...
Если у нас словарь из $$N$$ слов, то самый простой вариант – завести по одному признаку на каждое слово.

Документ тогда будет выглядеть как вектор длины V:
...
<div align="left"><figure><img src="../.gitbook/assets/18.1-documents-as-points-in-feature-space.png" alt=""><figcaption><p>18.1 Документы как точки в пространстве признаков</p></figcaption></figure></div>
...
Интересный момент: большинство координат в таких векторах равны нулю. Ведь как правило в одном конкретном документе используется лишь малая часть словаря.

Это приводит нас к разреженным векторам и соответствующим разреженным представлениям – таким структурам данных, которые хранят только ненулевые значения. Без этого работать с текстами было бы вычислительно крайне неэффективно и практически неприемлемо.
...
<div align="left"><figure><img src="../.gitbook/assets/18.2-cosine-similarity-between-document-vectors.png" alt=""><figcaption><p>18.2 Косинусное сходство между векторами документов</p></figcaption></figure></div>
...
Теперь представим каждый текст в виде вектора частот слов ([bag-of-words](../vvedenie/zaklyuchitelnye-materialy/glossarii.md#bag-of-words-meshok-slov), его мы рассмотрим подробней в следующей главе).
...
Геометрически это две точки в пятимерном пространстве. Они очень близки: отличаются лишь по координатам, соответствующим одному слову.

Если мы посчитаем евклидово расстояние между ними, то оно будет небольшим, а косинусное сходство – высоким. Алгоритм "видит", что тексты почти одинаковы, хотя и не знает, что такое кот и пёс.
...
<summary><strong>Пример с расширенным словарём</strong></summary>

Наш новый словарь: $$\{ кот, пёс, спит, на, диване, автомобиль, сломался, дороге \}$$

Вектор частот слов для текста A:

$$x_A = (1, 0, 1, 1, 1, 0, 0, 0)$$

Вектор частот слов для текста B:

$$x_B = (0, 1, 1, 1, 1, 0, 0, 0)$$

Вектор частот слов для текста C:
...
<div align="left"><figure><img src="../.gitbook/assets/18.3-two-almost-identical-texts-as-close-points.png" alt=""><figcaption><p>18.3 Два почти одинаковых текста как близкие точки</p></figcaption></figure></div>

### Почему размерность – это не проблема

Когда мы говорим, что пространство слов может иметь 10 000 или даже 100 000 измерений, это часто звучит пугающе. Кажется, что такая математика должна быть невероятно сложной.
...
### Почему это работает (хотя выглядит довольно наивно)

На первый взгляд вышеуказанный подход "слово = признак" кажется грубым. Он игнорирует порядок слов, грамматику, иронию, сарказм и вообще половину того, что мы считаем смыслом.
...
И причина этого кроется в статистике. В реальных текстах слова распределены не случайно. Темы, эмоции и намерения проявляются через частоты и сочетания слов.
...
### Главное, что стоит понять из этой главы
...
В следующих главах мы будем усложнять это пространство, делая его менее наивным и более выразительным. Но идея отображения текста в $$\mathbb{R}^n$$ останется с нами до самого конца книги.

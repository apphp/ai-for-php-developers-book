...
Если мы возьмём слово "ключ", то в моделях типа [word2vec](https://ru.wikipedia.org/wiki/Word2vec) или [GloVe](https://en.wikipedia.org/wiki/GloVe) оно будет представлено одной фиксированной точкой. Неважно, идёт ли речь о металлическом ключе от двери, о криптографическом ключе или о ключевой идее в тексте. Вектор один. Контекст исчезает.
...
тот же самый токен "ключ" окажется ближе к "токен", "доступ", "шифрование" и "безопасность".

Это не разные слова и не разные правила. Это результат работы механизма самовнимание ([self-attention](../../vvedenie/glossarii.md#self-attention)).

Важно, что контекст – это не только ближайшие слова. Самовнимание позволяет слову учитывать информацию из любой части предложения, даже если между ними десятки токенов. Именно это принципиально отличает трансформеры от более ранних подходов, где влияние далёких слов быстро затухает.
...
### Самовнимание: идея без формул

Самовнимание можно объяснить без математики.
...
Именно это внимание ([attention](../../vvedenie/glossarii.md#attention-mekhanizm)) и даёт контекст.

При этом важно понимать: веса внимания не заданы заранее. Модель не знает "из коробки", какие слова важны, а какие нет. Эти веса она учится выставлять сама в процессе обучения, постепенно подстраиваясь под задачу и минимизируя ошибку. Здесь нет скрытого знания о языке – только оптимизация и большое количество примеров (очень большое).

### Немного математики – ровно столько, сколько нужно

Совсем без математики объяснить эту тему тяжело, тем не менее мы попытаемся обойти это с минимальной интуицией.
...
Название "Key" здесь, кстати, совпадение и не имеет отношения к слову "ключ" из примеров выше.

Самовнимание делает следующее:
...
В результате механизм внимания учитывает не только то, какие слова рядом, но и где они находятся.

### Много голов лучше, чем одна

На практике самовнимание работает не в одном, а сразу в нескольких параллельных проекциях. Это называется многоголовое внимание (multi-head attention).
...
Каждое слово имеет вес (`$context`) и вектор из двух чисел (`$values`).\
Цикл умножает вес слова на его вектор и **складывает вклад всех слов** по каждой координате.

По формуле:

* $$result[0] = 0.6 \cdot 1.0 + 0.9 \cdot 0.9 + 0.1 \cdot 0.1 = 1.42$$
* $$result[1] = 0.6  \cdot 0.2 + 0.9 \cdot 0.8 + 0.1 \cdot 0.0 = 0.84$$

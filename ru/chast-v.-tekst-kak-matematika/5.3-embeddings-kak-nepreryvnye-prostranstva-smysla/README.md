...
Ключевая идея embeddings звучит почти философски: смысл с это не правило, а положение. Модель больше не проверяет набор условий – она измеряет взаимное расположение объектов.
...
Важно заметить: нас интересует угол между векторами, а не их длина. Во многих практических задачах длина вектора либо нормализуется, либо играет второстепенную роль по сравнению с направлением. Однако в некоторых [embeddings-моделях](../../vvedenie/glossarii.md#embeddings-modeli) норма вектора тоже может содержать полезную информацию.
...
Embedding-модели обучаются так, чтобы объекты, встречающиеся в похожих контекстах, оказывались ближе друг к другу. Это обобщение старой идеи дистрибутивной семантики: \
...
В следующих главах мы будем использовать embeddings как базовый строительный блок, почти как фундаментальную абстракцию для представления смысла и близости – не задумываясь о том, как именно внутри устроена эта геометрия, но постоянно пользуясь ее свойствами.

...
* **Кейс 1. Поиск похожих документов (чистый PHP)**\
  Сравнение текстов через TF–IDF и cosine similarity для поиска близких по смыслу документов.
* **Кейс 2. Классификация отзывов: «положительный / отрицательный» (RubixML)**\
  Автоматическое определение тональности текста с помощью Bag of Words и TF–IDF.
* **Кейс 3. Автоматическая категоризация статей (RubixML)**\
  Распределение текстов по темам на основе ключевых слов и статистических весов.
* **Кейс 4. Детектор «спама» в формах обратной связи (чистый PHP)**\
  Выявление спам-сообщений по характерным словам и текстовым паттернам.
* **Кейс 5. Объяснимый поиск: «почему найден именно этот документ?» (чистый PHP)**\
  Разбор вклада слов и TF–IDF-весов для интерпретации результатов поиска.
* **Кейс 6. Сравнение Bag of Words и TF–IDF на одном примере**\
  Наглядное сопоставление двух подходов и анализ различий в итоговых признаках и результатах.

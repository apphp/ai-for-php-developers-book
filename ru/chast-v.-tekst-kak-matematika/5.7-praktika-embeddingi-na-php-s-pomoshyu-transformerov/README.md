...
* d – размерность эмбеддинга (например, 384, 768, 1024, 1536, 3072 и другие)

Ключевой момент здесь не в самой функции $$f$$, а в геометрии получившегося пространства. Семантически близкие тексты обычно получают похожие векторные представления.
...
* inference обычно воспроизводим и при одинаковой модели выдаёт практически одинаковые результаты для одного и того же входного текста.

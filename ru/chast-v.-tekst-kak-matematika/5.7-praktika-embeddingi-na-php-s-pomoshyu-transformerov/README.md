...
Inference, напротив, покрывает большую часть прикладных задач:
...
В реальных моделях sentence-transformers mean pooling обычно выполняется с учётом attention mask, чтобы служебные и padding-токены не влияли на итоговый эмбеддинг. Для простоты здесь показана упрощённая схема.

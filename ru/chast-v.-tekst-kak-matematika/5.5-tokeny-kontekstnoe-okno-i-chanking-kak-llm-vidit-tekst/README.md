...
Для человека текст состоит из букв, слов, предложений и абзацев. Для языковой модели всё выглядит иначе. Она никогда не видит документ целиком в привычном нам виде. Перед обработкой текст разбивается на специальные элементы – [токены](../../vvedenie/glossarii.md#tokens-tokeny). Именно токены становятся базовыми единицами информации для LLM.
...
Контекстное окно ([context window](../../vvedenie/glossarii.md#context-window-kontekstnoe-okno)) – это максимальное количество токенов, которое модель может одновременно видеть.
...
Чанкинг ([chunking](../../vvedenie/glossarii.md#chunking-chanking)) – это процесс разбиения документа на фрагменты, удобные для индексации, поиска и последующей передачи в модель.

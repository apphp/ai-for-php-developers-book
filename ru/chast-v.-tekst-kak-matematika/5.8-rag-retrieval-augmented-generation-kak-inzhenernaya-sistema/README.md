...
На самом деле модель не хранит знания в виде базы фактов и не выполняет поиск информации в момент ответа – она лишь продолжает последовательность токенов, опираясь на статистические зависимости, выученные при обучении.&#x20;
...
Использование только LLM здесь не позволяет гарантировать корректность ответа. Она может выглядеть убедительно, но:
...
1. Retrieval:\
   $$R(q)=\mathrm{TopK}_{d_i \in D}\left(\mathrm{sim}\left(E(q), E(d_i)\right)\right)$$
2. Context construction:\
   $$C = \mathrm{concat}\bigl(d_{i_1}, d_{i_2}, \dots, d_{i_k}\bigr)$$
3. Generation:\
   $$\text{answer} = \mathrm{LLM}(q, C)$$

Важно понять: LLM не видит весь корпус, она видит только контекст $$C$$, который мы ей дали.

### Retrieval: поиск как векторная задача

#### Почему не keyword-поиск

Классический поиск по словам (типа: LIKE, [TF-IDF](../../vvedenie/glossarii.md#tf-idf), BM25) ищет совпадения токенов. RAG же опирается на семантическую близость. При этом на практике многие production-системы используют гибридный поиск, объединяя BM25, векторный поиск и последующее ранжирование результатов.
...
{% hint style="info" %}
Размерность эмбеддинга определяется архитектурой трансформера: часто она кратна числу attention heads и отражает компромисс между выразительностью и вычислительной стоимостью.
...
Итоговая размерность эмбеддинга может совпадать с этой величиной, но во многих современных embedding-моделях используется дополнительный проекционный слой.
...
Во многих production-системах между retrieval и построением контекста добавляется этап reranking. На этом этапе найденные чанки повторно оцениваются более точной моделью, которая лучше понимает смысл запроса. Обычно быстрый векторный поиск сначала отбирает 50–100 кандидатов, после чего reranker оставляет лишь несколько наиболее релевантных. Такой подход позволяет значительно повысить качество итогового контекста без заметного увеличения времени ответа.
...
* не ищет информацию
* сама по себе не имеет механизма проверки фактов
...
При таком подходе языковая модель не хранит знания сама по себе. Она получает необходимые данные из внешних источников, анализирует их и на их основе формирует ответ. Поэтому LLM здесь выступает не как носитель информации, а как интеллектуальный интерфейс для работы с данными.

...
* на не имеет доступа к вашим документам, если они не переданы ей в контексте
...
{% hint style="info" %}
Размерность эмбеддинга определяется архитектурой трансформера: часто она кратна числу attention heads и отражает компромисс между выразительностью и вычислительной стоимостью. Однако современные embedding-модели могут использовать отдельный проекционный слой.

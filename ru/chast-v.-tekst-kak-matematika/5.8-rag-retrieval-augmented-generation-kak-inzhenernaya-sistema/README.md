...
Когда говорят, что LLM "знает всё", это удобная, но опасная метафора. На самом деле модель ничего не знает в момент ответа – она лишь продолжает последовательность токенов, опираясь на статистические зависимости, выученные при обучении. Именно поэтому в реальных продуктах почти всегда возникает вопрос:&#x20;
...
Классический поиск по словам (типа: LIKE, [TF-IDF](../../vvedenie/glossarii.md#tf-idf), BM25) ищет совпадения токенов. RAG же опирается на семантическую близость.
...
Именно поэтому RAG – это прежде всего инженерный подход к построению AI-систем. Языковая модель остаётся важной частью решения, но качество всей системы определяется не только выбранной LLM, а тем, как организованы хранение данных, поиск релевантной информации и формирование ответа.

...
### Автокодировщики. Сжатие данных, латентное пространство и восстановление входа
...
\[IMAGE PLACEHOLDER]\
&#xNAN;_&#x421;хема автоэнкодера: вход → encoder → латентное пространство → decoder → восстановленный выход_

Промпт для генерации картинки:

```
A clean neural network diagram showing an autoencoder:
left side: input layer (image pixels grid),
middle: shrinking layers into a small bottleneck (latent vector),
right side: expanding layers reconstructing the image.
Minimalistic, educational style, white background.
```
...
Latent vector (например, 16 чисел) – это не просто «сжатые пиксели».\
Это абстрактные признаки:
...
Реализуем очень упрощённый автоэнкодер (без оптимизации, просто идея).
...
x\_{noisy} \rightarrow x\_{clean}
...
### Визуализация латентного пространства (плейсхолдер)

\[IMAGE PLACEHOLDER]\
&#xNAN;_&#x422;очки в 2D latent space, где похожие объекты расположены рядом_

Промпт:

```
2D latent space visualization: clusters of points representing handwritten digits, each cluster corresponds to a digit class, smooth transitions between clusters, soft colors, scientific visualization style
```
...
Автокодировщики уже намекнули на важную идею:

а что если не просто сжимать данные, а генерировать новые?

Следующий шаг – модели, которые не просто восстанавливают вход, а создают что-то новое.\
И здесь мы переходим к генеративным моделям и VAE глубже.

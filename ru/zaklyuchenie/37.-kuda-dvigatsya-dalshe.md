...
Без линейной алгебры невозможно по-настоящему понять эмбеддинги, [матричную факторизацию](../vvedenie/glossarii.md#matrichnaya-faktorizaciya) и нейронные сети.
...
Это стандартная функция потерь бинарной классификации (binary [cross-entropy](../vvedenie/glossarii.md#cross-entropy-kross-entropiya)), которая используется в логистической регрессии и нейронных сетях. Понимание того, как эта функция оптимизируется и почему градиентный спуск находит хороший минимум, дает уверенность, что модель обучается стабильно.

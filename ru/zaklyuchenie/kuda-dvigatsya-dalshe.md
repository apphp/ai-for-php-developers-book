...
Это стандартная функция потерь бинарной классификации (binary cross-entropy), которая используется в логистической регрессии и нейронных сетях. Понимание того, почему эта функция выпуклая и почему градиентный спуск находит глобальный минимум – дает уверенность в том, что модель обучается предсказуемо и стабильно.
...
А затем сделать это ещё раз – уже лучше.
...
Но в реальной индустрии эти роли редко существуют изолированно. Обычно они соединяются в одной архитектуре ML-систем.
...
И, возможно, самый зрелый этап развития – это когда вы начинаете видеть связь между формулой, кодом и продакшен-системой как единой инженерной системой.

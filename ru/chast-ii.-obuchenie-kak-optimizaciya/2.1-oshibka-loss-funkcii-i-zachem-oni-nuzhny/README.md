...
Важно понять одну существенную вещь: модель не знает, что такое "хорошо" и "плохо". Она не понимает смысл задачи. Всё, что она делает во время обучения – пытается уменьшить специальное число, которое мы считаем по её ошибке. Это число называется [loss](../../vvedenie/glossarii.md#loss-funkciya-funkciya-poter) (функция потерь).\
\
...
С точки зрения геометрии, [MSE](../../vvedenie/glossarii.md#mse-mean-squared-error) – это средний квадрат расстояния между реальными значениями и предсказаниями (то есть квадрат вертикальных отклонений точек от модели).
...
Формально это частный случай [кросс-энтропии](../../vvedenie/glossarii.md#cross-entropy-kross-entropiya):

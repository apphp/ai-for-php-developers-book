...
Когда мы говорим, что модель обучается, на самом деле мы имеем в виду очень простую идею: шаг за шагом уменьшать ошибку. Неважно, идёт ли речь о линейной регрессии, логистической модели или глубокой нейросети – почти всегда за обучением стоит один и тот же базовый механизм.
...
В этот момент полезно сменить абстракцию.&#x20;

Перестать думать об ошибке как о формуле и начать думать о ней как о ландшафте.
...
Он состоит из частных производных: $$∇L = (\frac{dL}{dw_1}, \frac{dL}{dw_2})$$
...
{% hint style="info" %}
[Градиент](../../vvedenie/zaklyuchitelnye-materialy/glossarii.md#gradient) = набор производных по всем переменным
{% endhint %}
...
Возьмём простую функцию: $$L(w) = (w - 3)^2$$

График этой функции выглядит как "чашка", минимум в точке $$w = 3$$.

Производная: $$\frac{dL}{dw} = 2 (w - 3)$$
...
{% hint style="info" %}
Градиентный идёт против градиента, чтобы спуститься в минимум (найти лучшие параметры)
{% endhint %}

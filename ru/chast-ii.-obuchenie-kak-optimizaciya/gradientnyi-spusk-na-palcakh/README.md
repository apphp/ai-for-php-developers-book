...
Этот механизм называется [градиентным спуском](../../vvedenie/zaklyuchitelnye-materialy/glossarii.md#gradientnyi-spusk).

Название может звучать непонятно, но сама идея очень проста и интуитивна. В её основе лежит понятие производной: она показывает направление, в котором функция растёт быстрее всего. А значит, если мы хотим уменьшить значение функции ошибки, двигаться нужно в противоположную сторону – против [градиента](../../vvedenie/zaklyuchitelnye-materialy/glossarii.md#gradient).
...
Представим себе очень простую ситуацию. У нас есть модель с одним параметром $$w$$. Мы меняем $$w$$ и каждый раз считаем ошибку $$L(w)$$. Например, это может быть [MSE](../../vvedenie/zaklyuchitelnye-materialy/glossarii.md#mse-mean-squared-error).
...
* если производная равна нулю, то мы либо в минимуме, либо в максимуме, либо на плато (в многомерном случае – также в [седловой точке](../../vvedenie/zaklyuchitelnye-materialy/glossarii.md#sedlovaya-tochka))
...
Здесь $$\eta$$ (эта буква называется "эта") – это скорость обучения, или [learning rate](../../vvedenie/zaklyuchitelnye-materialy/glossarii.md#learning-rate-skorost-obucheniya).
...
<summary>Очень простое интуитивное объяснение про производную, градиент и градиентный спуск</summary>
...
{% hint style="info" %}
То есть производная – это скорость изменения по одной переменной.
{% endhint %}

#### 2. Теперь про градиент

А теперь представьте, что вы не на линии, а на целой поверхности (как в 3D).

Например, у вас есть функция потерь: $$L(w₁, w₂)$$

Это уже не линия, а "ландшафт" – холмы и впадины.

Градиент – это: вектор (стрелка), который показывает, куда идти, чтобы быстрее всего подняться вверх.

Он состоит из производных: $$∇L = (\frac{dL}{dw_1}, \frac{dL}{dw_2})$$

То есть:

* $$\frac{dL}{dw_1}$$ – насколько круто в направлении $$w_1$$
* $$\frac{dL}{dw_2}$$ – насколько круто в направлении $$w_2$$

{% hint style="info" %}
Градиент = набор производных по всем переменным
{% endhint %}
...
Возьмём простую функцию: $$L(w) = (w - 3)^4$$

График этой функции выглядит как "чашка", минимум в точке w = 3.

Производная: $$\frac{dL}{dw} = 4 (w - 3)^3$$
...
Теперь функция: $$L(w₁, w₂) = (w₁ - 3)² + (w₂ + 1)²$$

Градиент: $$∇L = (2(w₁ - 3), 2(w₂ + 1))$$
...
#### 6. Интуиция для ребёнка

* Производная – это "насколько круто вперёд/назад"
* Градиент – это "стрелка в пространстве: куда идти, чтобы быстрее всего подняться"

{% hint style="info" %}
А градиентный спуск делает наоборот: идёт против градиента, чтобы спуститься в минимум (найти лучшие параметры)
{% endhint %}
...
3. много раз (эпох – [epochs](../../vvedenie/zaklyuchitelnye-materialy/glossarii.md#epoch-epokha)) считает ошибку,
...
* упрощает [backprop](../../vvedenie/zaklyuchitelnye-materialy/glossarii.md#backpropagation-obratnoe-rasprostranenie-oshibki),

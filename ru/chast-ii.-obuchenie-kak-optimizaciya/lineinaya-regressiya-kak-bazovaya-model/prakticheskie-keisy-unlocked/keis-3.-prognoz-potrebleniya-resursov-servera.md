...
$$
\hat{y} = w_1 \cdot x_1 + w_2 \cdot x_2 + w_3 \cdot x_3 + w_4 \cdot x_4 + w_5 \cdot x_5 + b
$$
...
В DevOps-кейсе данные со временем меняются. Добавляются новые фичи, меняется профиль трафика, появляются кэши и оптимизации. В инфраструктурных системах это проявляется как data drift и изменение зависимости между метриками ([concept drift](../../../vvedenie/zaklyuchitelnye-materialy/glossarii.md#concept-drift) в инженерном смысле).
...
Линейная регрессия не умеет ловить резкие скачки, нелинейные эффекты и [saturation-поведение](../../../vvedenie/zaklyuchitelnye-materialy/glossarii.md#saturation-povedenie) (например, когда CPU упирается в 100%).

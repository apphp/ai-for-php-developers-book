...
На практике часто смотрят только на accuracy: сколько объектов модель угадала правильно. Это удобно, интуитивно и… опасно. В этом кейсе мы покажем ситуацию, где две модели имеют одинаковую точность, но при этом одна из них объективно лучше как вероятностная модель, и это видно только через loss-функцию.
...
Две модели выдают вероятности принадлежности к классу 1.
...
Если мы применим стандартный порог 0.5, обе модели дадут одинаковые классы:
...
Используем стандартную бинарную log loss:
...
Но с точки зрения вероятностей:

* Model A говорит: «Я почти уверен».
* Model B говорит: «Я скорее думаю, но не ручаюсь».
...
Log loss – это не просто мера ошибки, а штраф за уверенность.
...
В этом кейсе Model A:

* уверена и права;
* получает меньший loss.

<br>

Model B:

* менее уверена, хотя тоже права;
* платит за это более высоким loss.
...
Log loss отвечает на другой вопрос:
...
* кейс «одинаковый log loss – разный accuracy»;
* или связка с calibration / reliability diagram;
* или короткая философская вставка про «модель, которая знает, что она не уверена».

...
Любая модель машинного обучения сводится к простой идее: она пытается описать реальность с помощью функции. Это значит, что между тем, что есть на самом деле и тем, что говорит модель - всегда будет расхождение. Это расхождение мы и называем ошибкой.
...
С точки зрения геометрии, [MSE](../../vvedenie/zaklyuchitelnye-materialy/glossarii.md#mse-mean-squared-error) – это это средний квадрат расстояния между реальными значениями и предсказаниями (то есть квадрат вертикальных отклонений точек от модели).
...
Если использовать MSE, разница между вероятностями 0.99 и 0.51 оказывается не такой значительной, хотя интуитивно это предсказания совершенно разного качества, если правильный ответ это 1.

...
Важно понять одну существенную вещь: модель не знает, что такое "хорошо" и "плохо". Она не понимает смысл задачи. Всё, что она делает во время обучения – пытается уменьшить специальное число, которое мы считаем по её ошибке. Это число называется [loss](../../vvedenie/5.-glossarii.md#loss-funkciya-funkciya-poter) (функция потерь).\
\
...
С точки зрения геометрии, [MSE](../../vvedenie/5.-glossarii.md#mse-mean-squared-error) – это средний квадрат расстояния между реальными значениями и предсказаниями (то есть квадрат вертикальных отклонений точек от модели).
...
Иначе говоря, MSE – это не просто удобная формула. Используя её, мы молчаливо предполагаем, что шум $$\varepsilon$$ подчиняется [нормальному распределению](../../vvedenie/5.-glossarii.md#gaussovo-raspredelenie) – тому самому "колоколу Гаусса".
...
Формально это частный случай [кросс-энтропии](../../vvedenie/5.-glossarii.md#cross-entropy-kross-entropiya):

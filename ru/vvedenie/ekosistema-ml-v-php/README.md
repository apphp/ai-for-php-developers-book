...
Рассмотрим простой и показательный пример. В нём мы обучаем классификатор $$k$$-ближайших соседей ([k-NN](../zaklyuchitelnye-materialy/glossarii.md#k-nn-k-nearest-neighbors-k-blizhaishikh-sosedei)) на небольшом наборе точек, каждая из которых принадлежит одному из двух классов – $$a$$ или $$b$$. После обучения модель должна определить, к какому классу относится новая точка.
...
Ключевая особенность библиотеки – локальный [инференс](../zaklyuchitelnye-materialy/glossarii.md#inferens-inference). Модели загружаются и выполняются на стороне PHP-приложения (через [ONNX Runtime](https://onnxruntime.ai/)), что открывает важные архитектурные возможности:
...
NeuronAI – это современный фреймворк для построения AI-приложений на PHP, ориентированный на агентный подход и orchestration LLM.

Если LLPhant можно рассматривать как PHP-аналог [LangChain](https://www.langchain.com/), то NeuronAI поднимается на уровень выше и описывает уже не отдельные вызовы модели, а поведение системы в целом.

Фреймворк предоставляет:

* агентную модель (agents)
* инструменты (tools) и их декларативное описание
* workflow и orchestration
* интеграцию с LLM-провайдерами
* управление контекстом и состоянием

Здесь важно не только получить ответ от модели, но и определить, какие шаги нужно выполнить, когда именно обращаться к LLM и как использовать внешние инструменты. Модель становится частью процесса, а не его центром.

NeuronAI вводит для этого более структурированный подход. Разработчик описывает агентов, доступные им инструменты и последовательность действий, после чего выполнение сценария берет на себя фреймворк. Это позволяет уйти от ручного управления цепочками вызовов и сделать поведение системы более предсказуемым и воспроизводимым.

Такой подход особенно полезен в задачах, где одного запроса к модели недостаточно: в многошаговых сценариях, автоматизации процессов и построении AI-ассистентов, взаимодействующих с внешними сервисами.
...
Отдельного упоминания заслуживает [ONNX](../zaklyuchitelnye-materialy/glossarii.md#one-hot-encoding). Через расширения или внешние сервисы PHP может выполнять инференс моделей, обученных в Python и экспортированных в ONNX-формат.

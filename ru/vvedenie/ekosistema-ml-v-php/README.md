...
Репозиторий: [https://github.com/ankane/mitie-php](https://github.com/ankane/mitie-php)

Статус: <mark style="color:green;">**поддерживается ограниченно**</mark>

MITIE PHP – это PHP-обёртка над библиотекой [MITIE](https://github.com/mit-nlp/mitie) (MIT Information Extraction), предназначенной для решения классических задач обработки естественного языка (NLP).

В отличие от современных трансформерных моделей, MITIE относится к поколению статистических NLP-систем, появившихся ещё до революции BERT и GPT. Несмотря на возраст подхода, библиотека до сих пор может быть полезна в проектах, где важны локальная обработка данных, минимальные требования к ресурсам и отсутствие внешних зависимостей.

MITIE позволяет решать такие задачи, как:

* распознавание именованных сущностей (Named Entity Recognition, NER)
* извлечение имен людей, компаний и географических объектов
* классификация текста
* анализ коротких документов

Все вычисления выполняются локально, без обращения к внешним API и облачным сервисам.

Простейший пример извлечения сущностей:
...
Результат будет содержать найденные сущности и их типы:
...
Архитектурно MITIE занимает промежуточное положение между классическим машинным обучением и современными NLP-системами. Он не использует трансформеры, эмбеддинги и attention-механизмы, поэтому обычно уступает современным моделям по качеству понимания языка. Однако взамен получает более высокую скорость работы и существенно меньшие требования к памяти и вычислительным ресурсам.

Сегодня MITIE редко рассматривают как основу для новых NLP-проектов, однако библиотека остаётся интересным примером локального NLP-инференса и показывает, каким образом задачи извлечения сущностей и классификации текста решались до появления современных трансформерных архитектур.
...
> Если MITIE представляет классическое поколение NLP-инструментов, то TransformersPHP переносит в PHP уже современные трансформерные архитектуры и контекстные представления текста.
...
По сути, это PHP-ориентированная обертка над идеями [Hugging Face Transformers](https://huggingface.co/docs/transformers/index), адаптированная под PHP-экосистему и реальные прикладные сценарии.

Ключевая особенность библиотеки – локальный [инференс](../glossarii.md#inferens-inference). Модели загружаются и выполняются на стороне PHP-приложения (через [ONNX Runtime](https://onnxruntime.ai/)), что открывает важные архитектурные возможности:
...
По мере развития AI-систем становится очевидно, что работа с моделью – это лишь небольшая часть общей архитектуры.

В реальных приложениях необходимо не только отправить запрос к модели и получить ответ, но и организовать работу с контекстом, памятью, документами, внешними инструментами, векторными хранилищами и бизнес-процессами.

Именно для решения этих задач появились AI-фреймворки.

Если библиотеки машинного обучения работают на уровне алгоритмов, а инфраструктурные решения отвечают за выполнение моделей, то AI-фреймворки поднимаются ещё на один уровень выше и позволяют строить полноценные AI-системы.

В экосистеме PHP наиболее заметными представителями этого направления являются LLPhant и Neuron AI.

#### **LLPhant**

Репозиторий: [https://github.com/LLPhant/LLPhant](https://github.com/LLPhant/LLPhant)
...
Сайт: [https://www.neuron-ai.dev](https://www.neuron-ai.dev/)
...
Если LLPhant можно рассматривать как PHP-аналог [LangChain](https://www.langchain.com/), фокусирующийся на интеграции LLM в приложение – чатах, эмбеддингах, vector store и RAG, – то Neuron AI поднимается на уровень выше и описывает уже не отдельные вызовы модели, а поведение системы в целом.
...
Отдельного упоминания заслуживает [ONNX](../glossarii.md#onnx-open-neural-network-exchange). Через расширения или внешние сервисы PHP может выполнять инференс моделей, обученных в Python и экспортированных в ONNX-формат.

...
Рассмотрим простой и показательный пример. В нём мы обучаем классификатор $$k$$-ближайших соседей ([k-NN](../5.-glossarii.md#k-nn-k-nearest-neighbors-k-blizhaishikh-sosedei)) на небольшом наборе точек, каждая из которых принадлежит одному из двух классов – $$a$$ или $$b$$. После обучения модель должна определить, к какому классу относится новая точка.
...
Ключевая особенность библиотеки – локальный [инференс](../5.-glossarii.md#inferens-inference). Модели загружаются и выполняются на стороне PHP-приложения (через [ONNX Runtime](https://onnxruntime.ai/)), что открывает важные архитектурные возможности:
...
Отдельного упоминания заслуживает [ONNX](../5.-glossarii.md#one-hot-encoding). Через расширения или внешние сервисы PHP может выполнять инференс моделей, обученных в Python и экспортированных в ONNX-формат.

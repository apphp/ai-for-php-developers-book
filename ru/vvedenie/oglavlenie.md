# Оглавление



{% hint style="info" %}
**Статус разработки книги**

Книга пишется публично и находится в активной разработке

**Общий прогресс**

Готовност&#x44C;**:** 44% (92%)

🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟨🟨🟨🟩🟨🟨🟨🟨🟨🟨🟨🟨🟨⬜⬜🟨⬜🟨🟨🟨🟨🟩

**Статусы разделов**

🟩 - готово\
🟨 - в работе\
⬜ - не начато
{% endhint %}

### Введение

1.  **Дисклеймер**

    _О границах книги, допущениях и ответственности._
2. **Введение**\
   &#xNAN;_&#x41C;отивация, реалии и цели книги._
   1. От понимания – к математике и реализации
   2. Зачем PHP-разработчику AI
3. **Экосистема ML в PHP**\
   &#xNAN;_&#x41E;бзор экосистемы PHP для машинного обучения и научных вычислений._
   1. Настройка среды для PHP
4. **О книге**
   1. Лицензия и авторские права
   2. Как контрибьютить
   3. История изменений книги
   4. Что дальше
5. **Глоссарий**
6. **Ресурсы и литература**

### Часть I. Математический язык AI

7. **Что такое модель в математическом смысле**\
   &#xNAN;_&#x424;ункция, параметры, ошибка._
8. **Векторы, размерности и пространства признаков**\
   &#xNAN;_&#x41F;очему данные – это точки в пространстве._
9. **Расстояния и сходство**\
   &#xNAN;_&#x415;вклидово расстояние, dot product, cosine similarity._
   1. Практические кейсы

### Часть II. Обучение как оптимизация

10. **Ошибка, loss-функции и зачем они нужны**\
    &#xNAN;_&#x4D;SE, log loss – без формального ада._
    1. Практические кейсы
11. **Линейная регрессия как базовая модель**\
    &#xNAN;_&#x424;ормула, геометрический смысл, PHP-реализация._
    1. Практические кейсы
12. **Градиентный спуск на пальцах**\
    &#xNAN;_&#x41F;очему производная – это направление движения._
    1. Эксперименты с градиентным спуском

### Часть III. Классификация и вероятности

13. **Вероятность как степень уверенности**\
    &#xNAN;_&#x427;астоты, апостериорные вероятности._
    1. Практические кейсы
14. **Логистическая регрессия**\
    &#xNAN;_&#x53;igmoid, decision boundary, кейс классификации._
    1. Сквозной кейс: распознавание цифр (MNIST)
    2. Практические кейсы
15. 🟨 Почему наивный Байес работает

    _Условные вероятности и независимость._

    1. 🟨 Практические кейсы

### Часть IV. Близость и структура данных

16. 🟨 Алгоритм k-ближайших соседей и локальные решения\
    &#xNAN;_&#x413;еометрическая интуиция, метрики расстояний._
    1. 🟨 Практические кейсы
17. 🟨 Decision Trees и разбиение пространства

    _Энтропия, information gain, объяснимость._

    1. 🟨 Практические кейсы

### Часть V. Текст как математика

18. **Почему слова превращаются в числа**\
    &#xNAN;_&#x41F;ространство слов и признаки._
19. 🟨 Bag of Words и TF–IDF

    _Формулы, интерпретация весов._

    1. 🟨 Практические кейсы
20. 🟨 Embeddings как непрерывные пространства смысла

    _Геометрия смысла и semantic search._

    1. 🟨 Практические кейсы
21. 🟨 Трансформеры и контекст: от статических векторов к пониманию смысла\
    &#xNAN;_&#x41F;очему слово "ключ" имеет разные векторы. Self-attention без формул._
    1. 🟨 Практические кейсы
22. 🟨 Named Entity Recognition (NER) – извлечение сущностей из текста\
    &#xNAN;_&#x53;equence labeling, BIO-разметка, практический кейс на PHP._
    1. 🟨 Практические кейсы
23. 🟨 Практика: эмбеддинги на PHP с помощью трансформеров\
    &#xNAN;_&#x49;nference вместо обучения._ TransformersPHP _как инженерный инструмент._
    1. 🟨 Практические кейсы
24. 🟨 RAG: Retrieval-Augmented Generation как инженерная система \
    &#xNAN;_&#x41F;оиск → контекст → генерация. Как LLM отвечает на основе документов._
    1. 🟨 Практические кейсы

### Часть VI. Attention и нейросети

25. 🟨 Перцептрон и полносвязная сеть\
    &#xNAN;_&#x41B;инейные комбинации и нелинейности._
    1. 🟨 Практические кейсы
26. 🟨 Backpropagation – почему он работает\
    &#xNAN;_&#x426;епное правило без академического ужаса._
27. 🟨 Attention как взвешенное суммирование

    _Формулы Q/K/V + упрощённая реализация._

    1. 🟨 Практические кейсы

### Часть VII. Обучение с подкреплением

28. ⬜ Обучение с подкреплением: как модели учатся через опыт \
    &#xNAN;_&#x410;гент, среда и награда. Обучение без правильных ответов._
    1. ⬜ Практические кейсы
29. ⬜ Почему обучение с подкреплением плохо подходит для текста\
    &#xNAN;_&#x418; почему язык – особый случай._

### Часть VIII. LLM и современный AI

30. 🟨 Почему LLM – это модели next-token prediction\
    &#xNAN;_&#x412;ероятности, softmax, контекст._
    1. 🟨 Практика: строим next-token модель на PHP
    2. 🟨 Практические кейсы
31. ⬜ Почему LLM ведут себя "разумно": RLHF и обучение поведения\
    &#xNAN;_&#x41E;т предсказания токенов к полезным ответам._
    1. ⬜ Практические кейсы
32. 🟨 Где LLM ошибаются математически\
    &#xNAN;_&#x413;аллюцинации, распределения, bias._

### Часть IX. Агентные системы и оркестрация

33. 🟨 Агентные системы и multi-step reasoning в PHP\
    &#xNAN;_&#x4C;LM как управляемая система: планирование, инструменты и контроль._
    1. 🟨 Практические кейсы

### Часть X. Продакшен и здравый смысл

34. 🟨 Как использовать AI в PHP-проектах\
    &#xNAN;_&#x410;рхитектуры, интеграции, orchestration._
35. 🟨 Когда AI не нужен и почему это важно

    _Понятные инженерные границы._

    1. 🟨 Практические кейсы

### Заключение

36. **Куда двигаться дальше**\
    &#xNAN;_&#x427;то углублять: математику, системы или практику._

...
Accuracy – это доля правильных предсказаний модели среди всех предсказаний.

Проще говоря, это процент случаев, где модель угадала.

Метрика проста и легко интерпретируется, но может вводить в заблуждение при несбалансированных данных. Например, если 95% объектов относятся к одному классу, модель может получить 95% accuracy, всегда предсказывая этот класс.

Поэтому accuracy обычно используют вместе с другими метриками.
...
Априорная вероятность – это вероятность события до учёта новых данных.\
Апостериорная – вероятность после учёта наблюдений.
...
Интуитивно: априорная – "что мы думали заранее", апостериорная – "что мы думаем после того, как увидели данные".
...
Практически любая модель сначала выполняет аффинное преобразование входных данных, а затем – переход к нелинейному представлению через функцию активации. Поэтому выражение $$Wx + b$$ можно считать фундаментальным строительным блоком современного ML.
...
### ANN-индексы (Approximate Nearest Neighbor)

ANN-индексы – это структуры данных для быстрого поиска ближайших векторов в высокоразмерном пространстве.

В отличие от точного поиска, ANN использует приближённые методы, чтобы значительно ускорить поиск с небольшой потерей точности.
...
Популярные ANN-алгоритмы: HNSW, IVF, PQ

Интуитивно: "найти почти самых близких соседей, но намного быстрее".
...
Auto encoders – это нейросетевые модели, которые учатся сжимать данные в компактное представление (код), а затем восстанавливать их обратно.
...
Используются для уменьшения размерности, удаления шума и обучения эмбеддингов.
...
Batch – это группа объектов, которая обрабатывается моделью за один шаг обучения.

Вместо работы со всеми данными сразу или по одному примеру, модель обновляет параметры по небольшим наборам данных.

Интуитивно: "маленькая порция данных для одного шага обучения".
...
Интуитивно: "обновляем исходную уверенность, учитывая новые наблюдения".
...
BM25 – это алгоритм поиска текста, который оценивает релевантность документа запросу на основе совпадений слов.

Он учитывает:

* частоту слова в документе (TF)
* редкость слова во всей коллекции (IDF)
* длину документа

В отличие от простого поиска по ключевым словам, BM25 лучше оценивает, насколько документ подходит запросу.

Используется в:

* поисковых системах
* гибридном поиске
* RAG-системах вместе с векторным поиском

Интуитивно: "насколько этот документ подходит запросу по словам и их важности".

### CART (Classification and Regression Trees)

CART – это алгоритм построения деревьев решений для задач классификации и регрессии.

Он рекурсивно разбивает данные по признакам, выбирая разбиения, которые лучше всего уменьшают "нечистоту" (например, через Gini impurity или MSE).

Особенности:

* бинарные разбиения (каждый узел делится на два)
* работает и для классификации, и для регрессии
* прост в интерпретации

Интуитивно: "задаём последовательность простых вопросов, чтобы разделить данные".

### Chain Rule (цепное правило )

Цепное правило – это правило математического анализа, которое позволяет вычислять производную сложной функции через производные входящих в нее функций. В нейронных сетях оно лежит в основе алгоритма обратного распространения ошибки (backpropagation): градиент ошибки последовательно передается от выходного слоя к входному, позволяя определить вклад каждого веса в итоговую ошибку.&#x20;

Благодаря цепному правилу нейронная сеть может эффективно вычислять градиенты даже при большом количестве слоев и параметров, что делает возможным обучение глубоких моделей.

### Chunking (чанкинг)

Chunking – это процесс разбиения большого документа на небольшие фрагменты (чанки), которые могут быть обработаны моделью отдельно.

Чанкинг используется, когда текст не помещается в контекстное окно или когда требуется более эффективный поиск по документам.
...
Качество разбиения напрямую влияет на качество поиска и ответов модели.

Интуитивно: "разрезаем большой текст на удобные для обработки куски".
...
### Content deduplication (удаление дубликатов контента)

Content deduplication – это процесс поиска и удаления повторяющихся или очень похожих данных.

В ML и NLP используется для:

* очистки обучающих данных
* уменьшения размера датасетов
* улучшения качества поиска и RAG-систем

Дубликаты могут приводить к:

* переобучению модели
* повторяющимся результатам поиска
* лишним затратам на хранение и обработку

Для поиска дублей используют:

* хеширование
* сравнение embedding-векторов
* similarity-метрики

Интуитивно: "убрать повторения, чтобы модель работала с более чистыми данными".

### Context Window (контекстное окно)

Context Window – это максимальное количество токенов, которое модель может одновременно учитывать при обработке запроса.

В контекстное окно входят:

* системные инструкции
* история диалога
* пользовательский запрос
* загруженные документы и другой контекст

Если объём данных превышает размер окна, часть информации приходится удалять, сокращать или разбивать на чанки.

Размер контекстного окна напрямую влияет на способность модели работать с длинными документами и поддерживать длинные диалоги.

Интуитивно: "контекстное окно – это объём памяти, доступный модели в текущий момент".

### Contextual embeddings (контекстные эмбеддинги)

Contextual embeddings – это векторные представления слов или текста, которые зависят от контекста, в котором они используются.

В отличие от обычных эмбеддингов, где одно слово имеет один и тот же вектор, контекстные эмбеддинги могут давать разные представления для одного и того же слова.

Например, слово "банк" будет иметь разные векторы в предложениях:
...
Интуитивно: "значение слова определяется не только самим словом, но и окружением".
...
### CRF-слой (Conditional Random Field, условное случайное поле)

CRF-слой – это слой, который учитывает зависимости между последовательными предсказаниями модели.

В отличие от обычной классификации, где каждый элемент оценивается независимо, CRF смотрит на всю последовательность и учитывает, какие метки лучше сочетаются друг с другом.

Часто использовался в NLP-задачах:

* Named Entity Recognition (NER)
* разметка частей речи
* извлечение информации из текста

Например, CRF может понять, что после метки начала сущности "B-PER" логично ожидать продолжение "I-PER", а не случайную другую метку.

Интуитивно: "модель учитывает не только отдельные ответы, но и то, как они связаны между собой".

### Cross-entropy (Кросс-энтропия)

Кросс-энтропия – это функция потерь, которая измеряет различие между истинным распределением вероятностей и распределением, предсказанным моделью.

Проще говоря, она показывает, насколько предсказанные вероятности отличаются от реальности.

Если модель даёт высокую вероятность правильному классу – кросс-энтропия мала.

Если модель уверенно ошибается – значение резко увеличивается.

Поэтому эта функция потерь особенно сильно наказывает уверенные ошибки.

Кросс-энтропия широко используется в задачах:
...
Интуитивно кросс-энтропия отвечает на вопрос: "Насколько распределение вероятностей модели похоже на реальное распределение?"

В практическом машинном обучении минимизация кросс-энтропии означает обучение модели, которая даёт корректные и хорошо откалиброванные вероятности классов.
...
### Дифференцируемые нейроны

Дифференцируемые нейроны – это нейроны, в которых функция активации имеет производную, позволяющую вычислять, как изменение входов и весов влияет на выход.&#x20;

Это необходимо для применения градиентного спуска и алгоритма обратного распространения ошибки (backpropagation). В отличие от классического персептрона с пороговой функцией, современные нейронные сети обычно используют дифференцируемые функции активации, такие как сигмоида, tanh и ReLU.
...
Embedding-вектор – это числовое векторное представление объекта в пространстве признаков.

Embedding-модели преобразуют слова, тексты, изображения или пользователей в такие векторы так, чтобы похожие по смыслу объекты находились близко друг к другу.

Embedding-векторы используются в:
...
Сходство между embedding-векторами обычно измеряют через косинусное сходство или расстояние между векторами.

Интуитивно: "embedding-вектор – это координаты смысла объекта в многомерном пространстве".

### Embeddings-модели

Embeddings-модели – это модели, которые преобразуют объекты (слова, тексты, изображения, пользователей) в векторы чисел.

Похожие по смыслу объекты получают близкие векторы в пространстве признаков.
...
Сходство между эмбеддингами обычно измеряют через cosine similarity или расстояние между векторами.

Интуитивно: "модель переводит смысл объектов в геометрию пространства".
...
Fairness – это свойство модели принимать решения без систематической дискриминации определённых групп.

В машинном обучении модель может показывать хорошую точность, но при этом работать хуже для отдельных категорий пользователей. Это может происходить из-за перекоса в данных, неправильных признаков или особенностей алгоритма.

Проблемы fairness возникают, когда модель:

* чаще ошибается для одной группы
* систематически занижает или завышает вероятность для определённых категорий
* обучается на исторических данных, содержащих социальные перекосы

В практике ML fairness рассматривают через разные критерии, например:

* одинаковая точность для разных групп
* одинаковые вероятности ошибок
* одинаковые условия принятия решений

Полностью удовлетворить всем критериям одновременно обычно невозможно, поэтому приходится выбирать баланс между fairness и другими метриками качества.

Интуитивно fairness отвечает на вопрос: "Не делает ли модель решения, которые систематически несправедливы для части пользователей?"

В современных ML-системах fairness рассматривается вместе с:
...
Forward pass – это этап работы нейронной сети, на котором входные данные последовательно проходят через все слои, а каждый нейрон вычисляет свое значение на основе входов, весов и функции активации. В результате сеть формирует итоговое предсказание.&#x20;

Во время прямого прохода веса модели не изменяются – он используется только для получения выхода. При обучении после прямого прохода вычисляется ошибка, которая затем используется алгоритмом обратного распространения ошибки ([backpropagation](glossarii.md#backpropagation-obratnoe-rasprostranenie-oshibki)) для корректировки весов.

### Gaussian Naive Bayes

Gaussian Naive Bayes – это вариант наивного Байеса для числовых признаков, в котором предполагается, что признаки внутри каждого класса распределены нормально (гауссово).

Модель оценивает для каждого класса:

* среднее значение признака
* дисперсию

И на основе этого вычисляет вероятность принадлежности объекта к классу.

Интуитивно: "значение признака вероятнее для того класса, где оно ближе к среднему с учётом разброса".
...
Gradient Boosting – это ансамблевый метод, в котором модели (обычно деревья) обучаются последовательно, исправляя ошибки предыдущих.

Каждая новая модель фокусируется на тех примерах, где предыдущие ошибались, и уменьшает функцию потерь с помощью градиентного спуска.

В итоге получается сильная модель из последовательности слабых.

Интуитивно: "каждый следующий шаг исправляет ошибки прошлого".
...
Latency – это время, за которое модель даёт ответ после получения входных данных.

Обычно измеряется в миллисекундах и особенно важна для real-time систем.

Интуитивно: "Насколько быстро модель отвечает".
...
LIKE – это простой алгоритм поиска по шаблону, используемый в базах данных для поиска строк по совпадению с частью текста.

Например: `WHERE name LIKE '%apple%'`

найдёт все записи, где встречается слово "apple".

Особенности:

* работает по символам, а не по смыслу
* не понимает синонимы и контекст
* прост в реализации

Используется для:

* простого поиска по полям базы данных
* фильтрации данных
* автоподбора в небольших системах

Интуитивно: "найти текст, который содержит нужный фрагмент".

### Logits

Logits – это необработанные значения, которые модель выдаёт перед преобразованием в вероятности.

Обычно это выход последнего слоя модели до применения функций вроде sigmoid или softmax. Например, в задаче классификации нейросеть сначала вычисляет набор чисел – logits. Затем они преобразуются в вероятности классов.
...
После этого получаются вероятности, сумма которых равна 1.

Важно, что logits сами по себе не являются вероятностями. Они могут быть любыми числами – положительными или отрицательными.

Logits удобны потому, что:

* с ними стабильнее считать функции потерь
* они напрямую используются в кросс-энтропии
* многие ML-библиотеки принимают именно logits, а не вероятности

Интуитивно logits можно представить как "сырые оценки уверенности" модели, которые затем переводятся в вероятности.

### Log-odds

Log-odds – это логарифм отношения вероятности события к вероятности его отсутствия:

$$
\log \frac{p}{1 - p}
$$

Он переводит вероятность из диапазона $$[0, 1]$$ в диапазон от $$-\infty$$ до $$+\infty$$.

Используется в логистической регрессии, где модель фактически предсказывает log-odds, а затем через sigmoid преобразует их в вероятность.

Интуитивно: log-odds – это "насколько вероятность склоняется в сторону события или его отсутствия".
...
LSTM – это разновидность [RNN](glossarii.md#rnn-recurrent-neural-network-rekurrentnaya-neironnaya-set), созданная для более эффективной работы с длинными последовательностями.

В отличие от обычных RNN, LSTM использует специальные механизмы памяти (gates), которые позволяют решать, какую информацию сохранить, а какую забыть.
...
LSTM уменьшает проблему затухающих градиентов и лучше сохраняет долгосрочный контекст, но при этом уступает transformer-моделям в работе с большими последовательностями.

Интуитивно: "модель умеет помнить важную информацию дольше и забывать ненужную".
...
MAE – это метрика для задач регрессии, которая показывает среднюю величину ошибки в абсолютных значениях.

Проще говоря, MAE отвечает на вопрос: "В среднем на сколько модель ошибается?"

Ошибка считается как абсолютная разница между предсказанием и реальным значением, без возведения в квадрат.

Особенности MAE:

* одинаково наказывает все отклонения
* менее чувствительна к выбросам, чем MSE
* измеряется в тех же единицах, что и целевая переменная

Сравнение с MSE:

* MSE сильнее штрафует большие ошибки (за счёт квадрата)
* MAE даёт более "устойчивую" оценку при наличии выбросов

Интуитивно:

MSE – "большие ошибки очень плохи"

MAE – "каждая ошибка важна одинаково"

MAE часто используют, когда:

* выбросы не должны доминировать
* важна интерпретируемость
* нужно понятное бизнесу измерение средней ошибки

В контексте оптимизации MAE сложнее для градиентных методов, потому что функция имеет излом в нуле (не гладкая), в отличие от MSE.
...
MLOps-инфраструктура – это набор процессов и инструментов для разработки, запуска и поддержки ML-моделей в production.

Она объединяет машинное обучение и бэкенд-разработку:
...
* model registry
* CI/CD для моделей
* мониторинг drift и метрик
* автоматическое переобучение

Интуитивно: "система, которая помогает модели жить после обучения".

### Momentum и адаптивные алгоритмы

Momentum – это техника, которая учитывает прошлые градиенты и добавляет "инерцию" к обновлениям. Это помогает быстрее двигаться в нужном направлении и уменьшает колебания при оптимизации.

Адаптивные алгоритмы (например, Adam) автоматически подбирают learning rate для каждого параметра. Они учитывают как среднее значение градиентов, так и их разброс, делая обучение более стабильным и быстрым.

Интуитивно:

* momentum – "ускоряемся в правильном направлении"
* Adam – "каждому параметру свой адаптивный шаг"
...
Multi-head attention – это расширение attention-механизма, в котором одновременно работают несколько независимых механизмов внимания ("голов").

Каждая голова может фокусироваться на разных аспектах данных: синтаксисе, семантике, дальних зависимостях или других закономерностях. Затем результаты объединяются в единое представление.

Multi-head attention является одним из ключевых компонентов transformer-моделей и позволяет лучше понимать сложные связи в тексте.

См. также: [Attention-механизм](glossarii.md#attention-mekhanizm), [Self-attention](glossarii.md#self-attention), [Transformer-модели](glossarii.md#transformer-modeli).

Интуитивно: "несколько наблюдателей одновременно смотрят на данные с разных точек зрения, а затем объединяют свои выводы".
...
Матричная факторизация – это метод, при котором большая матрица представляется как произведение нескольких меньших матриц.

В рекомендательных системах обычно используют матрицу "пользователь – объект" (оценки, клики, покупки). Она разреженная, потому что большинство взаимодействий неизвестны.

Метод разбивает её на две матрицы:

* векторы пользователей
* векторы объектов

Предсказание строится через скалярное произведение этих векторов.

Интуитивно модель находит скрытые факторы интересов пользователей и характеристик объектов.
...
NER – это задача NLP, в которой модель находит в тексте важные сущности и определяет их тип.
...
Пример: "Apple открыла офис в Лондоне"

Модель может выделить:
...
* анализе документов
* чат-ботах
* извлечении данных из текста

Интуитивно: "найти в тексте объекты, которые имеют смысл для системы".
...
PCA – это метод уменьшения размерности, который преобразует данные в новое пространство с меньшим числом признаков.

Он находит направления (главные компоненты), вдоль которых данные имеют наибольшую вариативность, и проецирует данные на них.

Интуитивно: "оставляем самое важное в данных, убирая лишнее".

### Pooling

Pooling – это способ получить одно векторное представление из набора векторов токенов.

В transformer-моделях каждый токен после обработки имеет свой embedding-вектор. Pooling объединяет эти векторы в один общий вектор предложения или документа.

Основные варианты:

* Mean pooling – усреднение всех embedding-векторов токенов. Часто используется в Sentence-Transformers.
* Max pooling – выбор максимальных значений по каждому измерению вектора.
* CLS token pooling – использование специального токена `[CLS]`, который модель обучает как общее представление всего текста (например, в BERT).

Pooling используется для:

* semantic search
* сравнения текстов
* классификации
* построения embedding-векторов

Интуитивно: "собрать много маленьких представлений токенов в один общий смысл текста".

### Positional encoding (позиционное кодирование)

Positional encoding – это способ добавить информацию о порядке элементов в transformer-модели.

Так как трансформер обрабатывает токены параллельно и сам по себе не знает порядок слов, к эмбедингу каждого токена добавляется информация о его позиции в последовательности.
...
Интуитивно: "модель получает не только смысл слова, но и его место в предложении".

### Правило обучения Розенблатта

Правило обучения Розенблатта – это алгоритм обучения персептрона, предложенный Фрэнком Розенблаттом в 1958 году. Его идея заключается в том, что веса модели изменяются только при ошибке классификации: если предсказание неверно, веса корректируются в направлении, уменьшающем вероятность повторения этой ошибки. Правило позволяет обучать персептрон распознавать линейно разделимые классы и стало одним из первых практических методов обучения искусственных нейронных сетей.
...
Quantile regression – это метод регрессии, который предсказывает не среднее значение, а конкретный квантиль распределения целевой переменной.

В обычной регрессии модель оценивает среднее значение y при заданных признаках.\
Квантильная регрессия позволяет оценивать, например:

* медиану (50-й перцентиль)
* нижнюю границу (например, 10-й перцентиль)
* верхнюю границу (например, 90-й перцентиль)

Это позволяет моделировать не только центральное значение, но и диапазон возможных результатов.

Пример:\
Если модель прогнозирует время доставки, можно отдельно оценить:

* типичное время доставки (медиана)
* быстрые доставки (10% самых быстрых)
* медленные доставки (90% случаев)

В отличие от MSE-регрессии, квантильная регрессия минимизирует асимметричную функцию потерь, которая по-разному штрафует недооценку и переоценку.

Плюсы:

* устойчива к выбросам
* показывает распределение возможных значений
* полезна для оценки рисков

Интуитивно quantile regression отвечает на вопрос: "Где находится нужная часть распределения результата, а не только его среднее значение".

Такие модели часто используют в:
...
Overlap – это повторение части текста в соседних чанках при разбиении документа в RAG-системах.

Перекрытие помогает сохранить контекст, если важная информация находится на границе двух чанков. Слишком маленький overlap может привести к потере смысла, а слишком большой – увеличить объём данных и число повторов.

Интуитивно: "соседние чанки немного перекрываются, чтобы не потерять важный контекст".

### **Precision, Recall и F1-score**

Precision (точность) – это доля правильных положительных предсказаний среди всех объектов, которые модель отнесла к положительному классу.&#x20;

Recall (полнота) – доля правильно найденных положительных объектов среди всех действительно положительных объектов.&#x20;

F1-score – гармоническое среднее Precision и Recall, которое позволяет оценить баланс между ними. Эти метрики особенно важны при несбалансированных классах, когда одной Accuracy может быть недостаточно для объективной оценки качества модели.

### RAG (Retrieval-Augmented Generation)

RAG – это архитектурный подход, который объединяет поиск информации и генерацию ответа с помощью LLM.

Вместо того чтобы полагаться только на знания, полученные моделью при обучении, RAG сначала ищет релевантные данные во внешних источниках, а затем передаёт найденный контекст модели.

Типичный RAG-пайплайн:

* документы разбиваются на чанки
* создаются embedding-векторы
* выполняется поиск похожих фрагментов
* найденный контекст добавляется в запрос к LLM

Используется для:

* работы с корпоративными документами
* AI-ассистентов
* поиска по базе знаний
* поддержки клиентов

Интуитивно: "сначала найти нужную информацию, потом попросить модель использовать её для ответа".

### Random Forest (случайный лес)

Random Forest – это ансамблевый метод, который строит множество деревьев решений и объединяет их предсказания.

Каждое дерево обучается на случайной подвыборке данных и признаков, поэтому деревья получаются разными. Итоговое решение:

* для классификации – голосование
* для регрессии – усреднение

Такой подход снижает переобучение и делает модель более устойчивой.

Интуитивно: "много слабых деревьев вместе дают сильное решение".

### Retrieval

Retrieval – это этап поиска релевантной информации перед её использованием моделью.

В RAG-системах retrieval получает запрос, находит наиболее подходящие документы или чанки и передаёт их LLM в качестве контекста.

Поиск может выполняться:

* по ключевым словам (например, BM25)
* по embedding-векторам (semantic search)
* гибридным способом

Качество retrieval напрямую влияет на качество итогового ответа модели.

Интуитивно: "сначала найти нужную информацию, затем использовать её для генерации ответа".

### Retrieval-архитектуры

Retrieval-архитектуры – это системы, в которых модель сначала ищет релевантные данные, а затем использует их для генерации ответа или принятия решения.

Обычно архитектура состоит из двух этапов:

* [retrieval](glossarii.md#retrieval) – поиск подходящих документов или объектов
* [inference](glossarii.md#inference-inferens) / generation – обработка найденного контекста моделью
...
Retrieval-архитектуры лежат в основе:
...
Интуитивно: "сначала найти нужную информацию, потом думать".

### RNN (Recurrent Neural Network, рекуррентная нейронная сеть)

RNN – это тип нейронных сетей, предназначенный для работы с последовательностями данных.

В отличие от обычных сетей, RNN имеет внутреннее состояние (память), которое переносится между шагами и позволяет учитывать предыдущие элементы последовательности.
...
Основная проблема RNN – сложность работы с длинным контекстом из-за затухающих градиентов. Позже эту проблему частично решили LSTM и GRU, а затем transformer-модели.

Интуитивно: "модель читает последовательность по шагам и сохраняет часть предыдущей информации".
...
Self-attention – это разновидность attention-механизма, при которой элементы последовательности анализируют связи друг с другом внутри того же входного контекста.

Для каждого слова модель определяет, на какие другие слова стоит обратить внимание и насколько сильно. Благодаря этому модель может учитывать как ближайший, так и очень далёкий контекст.

Self-attention является ключевым компонентом transformer-моделей и позволяет эффективно работать с длинными последовательностями.

См. также: [Attention-механизм](glossarii.md#attention-mekhanizm).

Интуитивно: "каждое слово смотрит на остальные слова и решает, какие из них важны для понимания контекста".

### Semantic search

Semantic search – это поиск по смыслу, а не только по точному совпадению слов.

Вместо обычного сравнения текста система преобразует запросы и документы в embedding-векторы и ищет близкие по смыслу объекты в векторном пространстве.

Semantic search позволяет находить:

* синонимы
* переформулировки
* похожие по смыслу тексты
* релевантные документы без точного совпадения слов

Обычно сходство измеряется через косинусное сходство или расстояние между векторами.

Интуитивно: "поиск пытается понять смысл запроса, а не просто совпадение слов".

### Sentence-Transformers

Sentence-Transformers – это модели, которые преобразуют целые предложения или небольшие тексты в embedding-векторы.

В отличие от обычных моделей, которые работают на уровне отдельных токенов, Sentence-Transformers создают одно компактное представление всего текста.
...
* поиска похожих сообщений

Похожие по смыслу предложения получают близкие векторы, которые можно сравнивать через [cosine similarity](glossarii.md#cosine-similarity-kosinusnoe-skhodstvo).

Интуитивно: "текст превращается в точку в пространстве смыслов".

### Sequence labeling (разметка последовательностей)

Sequence labeling – это задача, где модели нужно присвоить метку каждому элементу последовательности.

В NLP последовательностью обычно является текст, а элементами – токены.
...
Модель может разметить:

* Apple → ORGANIZATION
* открыла → O
* офис → O

Sequence labeling учитывает порядок элементов, поэтому часто используется вместе с:

* RNN / LSTM
* CRF-слоями
* transformer-моделями

Интуитивно: "дать каждому элементу последовательности правильную роль".

### Softmax

Softmax – это функция, которая преобразует набор чисел (logits) в вероятности.

Она используется в задачах многоклассовой классификации, когда модель должна выбрать один из нескольких классов.

На вход softmax получает logits – "сырые" значения модели.

На выходе получается распределение вероятностей, где:

* все значения лежат между 0 и 1
* сумма всех вероятностей равна 1

Это позволяет интерпретировать результат модели как вероятность принадлежности к каждому классу. Softmax усиливает различия между значениями: большие logits становятся ещё более вероятными, а маленькие – ещё менее.

Функция softmax тесно связана с:

* logits
* кросс-энтропией
* нейронными сетями
* вероятностной интерпретацией классификации

Интуитивно softmax отвечает на вопрос: "Как превратить набор оценок модели в распределение вероятностей по классам?"

### Sparse representation (разреженное представление)

Sparse representation – это представление данных, в котором большинство значений равно нулю.

В NLP это часто возникает в Bag of Words или TF-IDF, где документ содержит только небольшую часть слов из всего словаря.

Разреженные представления:

* экономят память
* ускоряют вычисления
* хорошо подходят для классических ML-алгоритмов

Но при очень большой размерности могут усложнять поиск сходства и обучение моделей.

Интуитивно: "вектор длинный, но почти все его значения пустые".

### Stochastic gradient descent (SGD)

Stochastic gradient descent  – это вариант градиентного спуска, при котором параметры модели обновляются не по всей обучающей выборке сразу, а по одному примеру или по небольшому случайному подмножеству данных.
...
Supervised learning – это подход, при котором модель обучается на размеченных данных: для каждого примера известен правильный ответ.

Задача модели – найти зависимость между входными признаками и целевой переменной, чтобы затем делать предсказания для новых данных.
...
Интуитивно: "модель учится на примерах с готовыми ответами".
...
Пороговая функция – это функция активации, которая преобразует непрерывное значение, вычисленное нейроном, в один из двух возможных выходов. Если сумма взвешенных входов превышает заданный порог, нейрон возвращает значение **1**, в противном случае – **0**.&#x20;

Такая функция использовалась в классическом персептроне Розенблатта для решения задач бинарной классификации. Несмотря на простоту, пороговая функция не является дифференцируемой, поэтому в современных нейронных сетях ее обычно заменяют гладкими функциями активации, такими как сигмоида, гиперболический тангенс или ReLU.
...
Токены – это базовые единицы текста, с которыми работает языковая модель.

Токеном может быть слово, часть слова, отдельный символ или знак пунктуации – в зависимости от используемого токенизатора.

Перед обработкой текст разбивается на токены, а затем преобразуется в числовые представления (эмбеддинги).

Количество токенов напрямую влияет на:

* размер контекстного окна
* стоимость использования модели
* скорость инференса

Важно понимать, что число токенов обычно не совпадает с числом слов.

Интуитивно: "токены – это строительные блоки текста, которые видит модель".
...
Transformer-модели – это нейросетевые архитектуры, основанные на attention-механизме.

В отличие от RNN, они обрабатывают все элементы последовательности параллельно и умеют учитывать дальние зависимости между словами.

Transformer лежит в основе современных NLP-моделей:

* GPT
* BERT
* T5
* Claude и многих других LLM.
...
Интуитивно: "модель смотрит на все слова сразу и определяет, какие из них важны друг для друга".

### Универсальный аппроксиматор

Универсальный аппроксиматор – это свойство нейронной сети с одним или несколькими скрытыми слоями приближать широкий класс непрерывных функций с произвольной точностью, если сеть имеет достаточное количество нейронов и подходящую функцию активации.&#x20;

Это свойство известно как теорема об универсальной аппроксимации. Она объясняет, почему даже относительно простая многослойная нейронная сеть способна моделировать сложные нелинейные зависимости, хотя сама теорема не гарантирует, что сеть будет легко обучаться или хорошо работать на новых данных.

### Vector Store (векторное хранилище)

Vector Store – это специализированное хранилище для embedding-векторов, оптимизированное для быстрого поиска похожих объектов.

Помимо самих векторов обычно хранится связанный с ними контент и метаданные.

Vector Store используется в:
...
Для быстрого поиска обычно применяются ANN-индексы.

Интуитивно: "база данных, в которой поиск выполняется по смысловой близости векторов, а не по совпадению текста".

### Энтропия

Энтропия – это мера неопределённости распределения.

Если все объекты относятся к одному классу – энтропия равна нулю (полная определённость).\
Если классы распределены равномерно – энтропия максимальна.

Формально: $$H = - \sum p_i \log p_i$$

В машинном обучении используется для:

* оценки "чистоты" данных
* построения деревьев решений
* вычисления information gain

Интуитивно: "насколько сложно угадать правильный класс".
